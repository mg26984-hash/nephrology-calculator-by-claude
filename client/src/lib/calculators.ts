...
// OPTN EPTS Mapping Table: raw EPTS score thresholds for each percentile (0-100%)
// Source: OPTN/SRTR reference population (transplantr R package, SRTR 2018 data)
// Reference: https://optn.transplant.hrsa.gov/media/g3xfajp4/epts_mapping_table.pdf
// Note: This table is updated annually by OPTN each spring.
const EPTS_MAPPING_TABLE: [number, number][] = [
  [0.01760937493806, 0],
  [0.26646040897072, 1],
  [0.43019022030158, 2],
  [0.53311909650924, 3],
...
function eptsLookup(rawEPTS: number): number {
  for (const [threshold, percentile] of EPTS_MAPPING_TABLE) {
    if (rawEPTS <= threshold) {
      return percentile;
    }
  }
  return 100;
...
export function epts(
  ...
  const rawScore = Math.round(rawEPTS * 10000) / 10000;
  const percentile = eptsLookup(rawEPTS);
